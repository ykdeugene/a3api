...
async function checkGroup(username, group_name) {
  return new Promise((resolve, reject) => {
    connectDatabase.getConnection(function (err, connection) {
      ...
      const query =
        "SELECT username FROM user_groups WHERE username = ? AND group_name = ?"
...
async function sendEmail(req, res, next) {
  ...
  connectDatabase.getConnection(function (err, connection) {
    ...
    const query1 =
      "SELECT username FROM tms_database.user_groups WHERE group_name = 'ProjectLead';"
...
exports.CreateTask = async (req, res, next) => {
  try {
    ...
    // MANDATORY FIELDS
    let validateAppName =
      validator.isAlpha(req.body.Task_app_Acronym) &&
      req.body.Task_app_Acronym !== "" // checks Task_app_Acronym is alphabets => T/F
    let validateTaskName =
      validator.isAscii(req.body.Task_name) && req.body.Task_name !== "" // checks Task_name is ASCII => T/F
    ...
    req.body.Task_description === ""
      ? (validateTaskDescription = true)
      : (validateTaskDescription = validator.isAscii(req.body.Task_description))
    req.body.Task_plan === ""
      ? (validateTaskPlan = true)
      : (validateTaskPlan = validator.isAscii(req.body.Task_plan))

    let validation =
      validateAppName &&
      validateTaskName &&
      validateTaskDescription &&
      validateTaskPlan // check all validation  => T/F
  ...
  connectDatabase.getConnection(async function (err, connection) {
    ...
    const appDetails = await new Promise((resolve, reject) => {
      const query1 =
        "SELECT App_Rnumber, App_permit_Create, App_Acronym FROM application WHERE App_Acronym = ?"
      ...
      connection.query(query1, data1, (err, results) => {
        ...
        try {
          resolve({
            currentRNumber: results[0].App_Rnumber,
            permissionCreate: results[0].App_permit_Create,
            App_Acronym: results[0].App_Acronym,
          }) // Task_app_Acronym is correct
        } catch (e) {
    ...
    const username = await checkGroup(
      req.body.Username,
      appDetails.permissionCreate
    )
    ...
    const Plan_MVP_name = await new Promise((resolve, reject) => {
      ...
      const query2 =
        "SELECT Plan_MVP_name FROM plan WHERE Plan_appAcronym = ? AND Plan_MVP_name = ?"
...
==============================`
    ...
    const query3 =
      "INSERT INTO task (`Task_name`, `Task_description`, `Task_notes`, `Task_id`, `Task_plan`, `Task_app_Acronym`, `Task_state`, `Task_creator`, `Task_owner`, `Task_createDate`) VALUES (?,?,?,?,?,?,'1',?,?,?)"
    const data3 = [
      req.body.Task_name,
      req.body.Task_description,
      taskNotes,
...
exports.GetTaskbyState = (req, res, next) => {
  try {
    ...
    let validateAppName =
      validator.isAlpha(req.body.Task_app_Acronym) &&
      req.body.Task_app_Acronym !== "" // checks Task_app_Acronym is alphabets => T/F
    let validateTaskState =
      validator.isAlpha(req.body.Task_state) &&
      ["Open", "toDo", "Doing", "Done", "Closed"].includes(req.body.Task_state) // checks Task_state is either ["Open", "toDoList", "Doing", "Done", "Closed"] => T/F
  ...
  const taskState =
    ["Open", "toDo", "Doing", "Done", "Closed"].indexOf(req.body.Task_state) + 1
  ...
  connectDatabase.getConnection(function (err, connection) {
    ...
    connection.query(query, data, (err, results) => {
      ...
      results.map((result) => {
        result.Task_state = ["Open", "toDo", "Doing", "Done", "Closed"][
          result.Task_state - 1
        ]
      })
...
exports.PromoteTask2Done = async (req, res, next) => {
  try {
    ...
    // MANDATORY FIELDS
    let validateTaskID =
      validator.isAlphanumeric(req.body.Task_id, "en-US", { ignore: "_" }) &&
      req.body.Task_id !== "" // checks Task_id is isAscii => T/F

    // OPTIONAL FIELDS
    let validateTaskNotes
    // checks Task_notes is ASCII => T/F
    req.body.Task_notes === ""
      ? (validateTaskNotes = true)
      : (validateTaskNotes = validator.isAscii(req.body.Task_notes))
  ...
  connectDatabase.getConnection(async function (err, connection) {
    ...
    const taskDetails = await new Promise((resolve, reject) => {
      const query1 =
        "SELECT Task_app_Acronym, Task_state, Task_notes FROM task WHERE Task_id = ?"
      ...
      connection.query(query1, data1, (err, results) => {
        ...
        try {
          resolve({
            appName: results[0].Task_app_Acronym,
            taskState: results[0].Task_state,
            taskNotes: results[0].Task_notes,
          }) // Task_app_Acronym is correct
        } catch (e) {
    ...
    const appPermissionDoing = await new Promise((resolve, reject) => {
      const query2 =
    ...
    const username = await checkGroup(req.body.Username, appPermissionDoing)
...
${taskDetails.taskNotes}`
    ...
    const query3 =
      "UPDATE task SET Task_notes = ?, Task_state = ?, Task_owner = ? WHERE Task_app_Acronym = ? AND Task_id = ?"
    const data3 = [
      taskNotesComplete,
      4,
      username,
